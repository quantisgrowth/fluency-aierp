  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!schoolId || memberError) {
        if (active) { setError("Não foi possível encontrar sua escola ativa."); setLoading(false); }
        return;
      }
      ...
      if (!active) return;
      if (schoolResult.error || unitResult.error) setError("Não foi possível carregar seu ambiente.");
      else { setSchool(schoolResult.data); setUnits(unitResult.data ?? []); }
      setLoading(false);
    }
    void load();
    return () => { active = false; };
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#07090e] px-4 py-10 text-white">
      <div className="w-full max-w-xl space-y-5 rounded-2xl border border-white/10 bg-neutral-900 p-8">
        ...
        {loading ? <p>Carregando seu ambiente…</p> : error ? <p>{error}</p> : school && (
          <>
            ...
            <p className="text-sm text-amber-300">
              O cadastro de alunos, turmas e financeiro ainda está em implantação.
              Os painéis demonstrativos não representam os dados da sua escola.
            </p>
          </>
        )}
        <button onClick={() => void signOut()} className="text-sm text-neutral-400 underline hover:text-white">
          Sair da conta
        </button>
      </div>
    </main>
